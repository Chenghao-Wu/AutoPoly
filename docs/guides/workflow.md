...
## Step 3 — Polymerization: how to build it
...
`Polymerization` takes the list of models and runs the full pipeline:
...
5. **Output organization** — `system.data`, `system.in.init`, `system.in.settings`, and `system.in.charges` are written to `<System out>/<Polymerization name>/`.
...
- [Agent API & CLI](agent.md) — the same workflow driven by a JSON config
